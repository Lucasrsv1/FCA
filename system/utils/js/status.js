function CreateGraphic (graphicNum) {
	...
	var format = "YYYY-MM-DD HH:00", formatBR = "";
	switch (dateGroup) {
		case 0:
	...
	$.ajax({
		...
		success: function (data) {
			...
			// Renderizar
			RenderGraphicMultiSeriesAndValues(graphicNum, title, series, [], values, 24, prefix, 0, false, secondaryData, zoom, lengths, nomes, startDate, endDate, formatBR, dateGroup, coresPorEvento);
		},
		error: function (data) {
			$('#graph-row' + graphicNum + ' .refresh').removeClass('fa-spin');
